...
def app2():
    ...
    ticker_list = ['RELIANCE.NS',
     ...
     'ITDC.NS',
     'APARINDS.NS']
    st.markdown("** PriceActionTradeAutomation **")
    ...
    def add_MA_and_EMA(df,stock_list):
        for stock in stock_list:
            ...
            df[stock,'44ma'] = df[stock,'Close'].rolling(window=44, min_periods=0).mean()
            #adding 5day expopential moving average
            df[stock,'5ema'] = df[stock,'Close'].ewm(span=5, adjust=False).mean()
            df=df.sort_index(axis=1)
        return df
    #Swing Trading
    ...
    risk=st.sidebar.number_input("Enter the risk value for your stocks(INR):")
    if st.button("Swing Trade", key="1"):
