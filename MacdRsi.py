...
def app3():
    ...
    def getMultipleStockData(stock_list,timeframe):
        ...
        if timeframe in timeSpan:
            ...
            p='max'
        df = yf.download(stock_list, interval=timeframe, period=p, group_by='tickers')
        df.dropna(inplace=True)
    ...
    if st.button("MacdRsi Trade"):
        ...
        trade_df=pd.DataFrame(list(zip(ticker_names,entry_prices)),
                                columns=['Ticker Name','Entry Price'])
        st.write(trade_df)
